...
#------- model1 -------------------
model1 <- glmmTMB::glmmTMB(log(body_condition) ~ mean_sst_celsius + percent_mpas_all + mean_gravity + 
...
#------- model1poly -------------------
model1poly <- glmmTMB::glmmTMB(log(body_condition) ~ poly(mean_sst_celsius, 2) + poly(percent_mpas_all, 2) + poly(mean_gravity, 2) + 
                                 type_stage_refined + month + (1|indiv_id), data = dat_model, family = gaussian)
#intercept varying among type_stage_refined and among indiv_id within type_stage_refined
...
#------- model2 -------------------
model2 <- glmmTMB::glmmTMB(log(body_condition) ~ gdp_per_capita_province + percent_mpas_all + mean_gravity + 
                             type_stage_refined + month + (1|indiv_id), data = dat_model, family = gaussian)
#intercept varying among type_stage_refined and among indiv_id within type_stage_refined
...
#------- model2poly -------------------
model2poly <- glmmTMB::glmmTMB(log(body_condition) ~ poly(gdp_per_capita_province, 2) + poly(percent_mpas_all, 2) + poly(mean_gravity, 2) + 
                                 type_stage_refined + month + (1|indiv_id), data = dat_model, family = gaussian)
#intercept varying among type_stage_refined and among indiv_id within type_stage_refined
...
webshot2::webshot(html_file, file = "summary_capture.png", delay = 5, width = 500)
...
sink(here::here("outputs", "7-join_explore_environmental_mes_dat_final_img", paste0("test_anova_models.txt")))
stats::anova(model1, model1poly, model2, model2poly, test = "Chisq")
sink(NULL)

sink(here::here("outputs", "7-join_explore_environmental_mes_dat_final_img", paste0("AIC_models.txt")))
bbmle::AICtab(model1, model1poly, model2, model2poly)
sink(NULL)

#nice way to display summary for all model
dir_name <- here::here("outputs", "7-join_explore_environmental_mes_dat_final_img")
sjPlot::tab_model(model1, model1poly, model2, model2poly, show.reflvl = TRUE, p.style = "stars", digits.re = 4, file = here::here(dir_name, paste0("tab_models_1_1poly_2_2poly.html")))

#take screenshot of summary 
html_file <- here::here(dir_name, paste0("tab_models_1_1poly_2_2poly.html"))
webshot2::webshot(html_file, file = "tab_model_capture.png", delay = 4, width = 3000)
